...
| `/saved` | `SavedLibraryView` presents Due, Weak, New, Learning, Mastered, and All tabs from saved words and review-state evidence. | Conditional private OK. Public needs account persistence and large-library QA. |
...
| `/saved` | Good: memory queue framing is present. | Medium: tabbed filters and card metadata can grow dense. | Strong: Review now and bulk review links drive events. | Strong: saved words become queue items. | Strong: tabs derive from review state. | Mostly absent, appropriate. | Large libraries and 320px layouts need QA. | Low. |
